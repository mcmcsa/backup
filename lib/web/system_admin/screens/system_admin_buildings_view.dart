...
class SystemAdminBuildingsView extends StatefulWidget {
  const SystemAdminBuildingsView({super.key});

  @override
  State<SystemAdminBuildingsView> createState() =>
      _SystemAdminBuildingsViewState();
}

class _SystemAdminBuildingsViewState extends State<SystemAdminBuildingsView> {
  // ── Data ──────────────────────────────────────────────────────────────────
  List<Building> _buildings = [];
  List<Department> _departments = [];
  Map<String, int> _roomCounts = {}; // building_id -> count
  bool _loading = true;
  String? _error;
  ...
  List<Building> get _filtered {
    ...
    var list = _buildings.where((b) {
      if (q.isNotEmpty) {
        final matchName = b.name.toLowerCase().contains(q);
        final matchCode = b.code.toLowerCase().contains(q);
  ...
  int get _active => _buildings.where((b) => b.isActive).length;
  int get _inactive => _buildings.where((b) => !b.isActive).length;
  ...
  void _showAddDialog() {
    showDialog(
      context: context,
      builder: (_) => _BuildingFormDialog(
        title: 'Add Building',
        existingBuildings: _buildings,
        departments: _departments,
        onSave: (name, code, deptId, floors) async {
          final err = await BuildingService.create(
            name: name,
            code: code,
            departmentId: deptId,
        ...
        onSuccess: () {
          ...
          _load();
          _toast('Building added successfully.');
        },
      ),
    );
  }

  void _showEditDialog(Building building) {
    showDialog(
      context: context,
      builder: (_) => _BuildingFormDialog(
        title: 'Edit Building',
        ...
        onSuccess: () {
          Navigator.pop(context);
          _load();
          _toast('Building updated.');
        },
      ),
    );
  }

  void _showDetailDialog(Building building) {
    showDialog(
      context: context,
      builder: (_) => _BuildingDetailDialog(
        building: building,
        roomCount: _roomCounts[building.id] ?? 0,
      ),
    );
  }

  Future<void> _toggleActive(Building building) async {
    final activate = !building.isActive;
    final confirmed = await _confirm(
      title: activate ? 'Restore Building' : 'Disable Building',
      message: activate
          ? 'Restore "${building.name}"? It will be available for new rooms and assignments.'
          : 'Disable "${building.name}"? It will be hidden from selection lists.',
      confirmLabel: activate ? 'Restore' : 'Disable',
      danger: !activate,
    );
    if (!confirmed) return;

    final err = await BuildingService.setActive(building.id, active: activate);
    if (err == null) {
      _toast(activate ? '${building.name} restored.' : '${building.name} disabled.');
      _load();
    } else {
  ...
  Future<void> _delete(Building building) async {
    final confirmed = await _confirm(
      ...
      message:
          'Permanently delete "${building.name}"?\n\nThis cannot be undone. Ensure no rooms or assets are tied to this building before deleting.',
      confirmLabel: 'Delete',
      danger: true,
    );
    if (!confirmed) return;

    final err = await BuildingService.deleteBuilding(building.id, building.name);
    if (err == null) {
      _toast('"${building.name}" has been deleted.');
      _load();
    } else {
      _toast('Error: $err', isError: true);
    }
    ...
    return Row(
      children: [
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text('Manage campus buildings and facilities.',
        ...
        ElevatedButton.icon(
          onPressed: _showAddDialog,
          icon: const Icon(Icons.add_rounded, size: 18),
          label: Text(isMobile ? 'Add' : 'Add Building'),
          style: ElevatedButton.styleFrom(
            backgroundColor: AdminStyles.primary,
            foregroundColor: Colors.white,
            padding: EdgeInsets.symmetric(
  ...
  Widget _buildStatCards(bool isMobile) {
    final cards = [
      _Stat('Total Buildings', _buildings.length,
          Icons.domain_rounded, AdminStyles.primary),
  ...
  Widget _buildTableHeader() {
    return Container(
      ...
      child: Row(
        children: [
          _sortableHeader('Building Name', 'name', flex: 4),
          _th('Code', flex: 2),
          _th('Floors', flex: 1, center: true),
          _th('Rooms', flex: 1, center: true),
          _th('Status', flex: 2),
  ...
  Widget _buildTableRow(Building building) {
    return Container(
      ...
      child: Row(
        children: [
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Center(
                    child: Icon(Icons.domain_rounded, color: AdminStyles.primary, size: 20),
                  ),
                ),
                const SizedBox(width: 12),
                Flexible(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (building.department.isNotEmpty)
                        Text(
                          building.department,
                          style: AdminStyles.bodyStyle(
          ...
          Expanded(
            flex: 2,
            child: _StatusChip(isActive: building.isActive),
          ),
          // Actions
          Expanded(
            flex: 2,
            child: _buildActions(building),
          ),
        ],
      ),
    );
  ...
  Widget _buildActions(Building building) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        _IconBtn(
          ...
          onTap: () => _showDetailDialog(building),
        ),
        ...
        PopupMenuButton<String>(
          ...
          onSelected: (v) {
            if (v == 'toggle') _toggleActive(building);
            if (v == 'delete') _delete(building);
          },
          itemBuilder: (ctx) => [
            PopupMenuItem(
              value: 'toggle',
              child: Row(children: [
                Icon(
                  ...
                  size: 18,
                  color: building.isActive ? AdminStyles.warning : AdminStyles.success,
                ),
                const SizedBox(width: 10),
                Text(building.isActive ? 'Disable' : 'Restore',
                    style: AdminStyles.bodyStyle(
  ...
  Widget _buildMobileCard(Building building) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(14),
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: const Center(
              child: Icon(Icons.domain_rounded, color: AdminStyles.primary, size: 24),
            ),
          ),
              ...
              children: [
                Row(
                  children: [
                    ...
                    _StatusChip(isActive: building.isActive, compact: true),
                  ],
                ),
                const SizedBox(height: 4),
                Text('Code: ${building.code}',
                    style: AdminStyles.bodyStyle(fontSize: 12, color: AdminStyles.textSecondary, fontWeight: FontWeight.w600)),
                if (building.department.isNotEmpty) ...[
                  const SizedBox(height: 4),
                ...
                Row(
                  children: [
                    Icon(Icons.layers_rounded, size: 12, color: AdminStyles.textMuted),
                    const SizedBox(width: 4),
                    Text('${building.numberOfFloors} Floors',
                        style: AdminStyles.bodyStyle(
                            fontSize: 11, color: AdminStyles.textMuted)),
                    const SizedBox(width: 12),
                    Icon(Icons.meeting_room_rounded, size: 12, color: AdminStyles.textMuted),
                    const SizedBox(width: 4),
                    Text('${_roomCounts[building.id] ?? 0} Rooms',
                        style: AdminStyles.bodyStyle(
                            fontSize: 11, color: AdminStyles.textMuted)),
                  ],
                ),
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (v) {
              switch (v) {
                case 'view':
                  _showDetailDialog(building);
                  break;
                case 'edit':
                  _showEditDialog(building);
                  break;
                case 'toggle':
                  _toggleActive(building);
                  break;
                case 'delete':
                  _delete(building);
                  break;
...
class _BuildingFormDialog extends StatefulWidget {
  ...
  final List<Building> existingBuildings;
  final List<Department> departments;
  final Future<String?> Function(String name, String code, String deptId, int floors) onSave;
  final VoidCallback onSuccess;

  const _BuildingFormDialog({
    required this.title,
    this.building,
    required this.existingBuildings,
...
class _BuildingFormDialogState extends State<_BuildingFormDialog> {
  ...
  late final _codeCtrl =
      TextEditingController(text: widget.building?.code ?? '');
  late final _floorsCtrl =
      TextEditingController(text: widget.building?.numberOfFloors.toString() ?? '1');
  ...
  void initState() {
    super.initState();
    if (widget.building != null && widget.building!.departmentId.isNotEmpty) {
      _selectedDeptId = widget.building!.departmentId;
    }
  }
  ...
  Future<void> _submit() async {
    ...
    final floors = int.tryParse(_floorsCtrl.text.trim()) ?? 1;
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: _inputDecor(Icons.business_outlined,
                      hint: 'e.g. Main Building'),
                  validator: (v) {
                    if (v == null || v.trim().isEmpty) return 'Required';
                    final dup = widget.existingBuildings.any((b) =>
                        b.id != (widget.building?.id ?? '') &&
                        ...
                        children: [
                          ...
                          TextFormField(
                            ...
                            style: AdminStyles.bodyStyle(
                                color: AdminStyles.textPrimary,
                                fontWeight: FontWeight.w600),
                            decoration: _inputDecor(Icons.code_rounded,
                                hint: 'e.g. MB'),
                            validator: (v) {
                              if (v == null || v.trim().isEmpty) return 'Required';
                              final dup = widget.existingBuildings.any((b) =>
                                  b.id != (widget.building?.id ?? '') &&
                                  b.code.trim().toLowerCase() ==
                                      v.trim().toLowerCase());
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: _saving
                          ...
                          : Text(
                              widget.building == null ? 'Add Building' : 'Save Changes',
...
class _BuildingDetailDialog extends StatelessWidget {
  final Building building;
  final int roomCount;

  const _BuildingDetailDialog({required this.building, required this.roomCount});
            ...
            children: [
              ...
              Row(children: [
                Container(
                  ...
                  child: const Center(
                    child: Icon(Icons.domain_rounded, color: AdminStyles.primary, size: 28),
                  ),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(building.name,
                          style: AdminStyles.headingStyle(fontSize: 18)),
                      const SizedBox(height: 4),
                      _StatusChip(isActive: building.isActive),
                    ],
                  ),
                ),
                IconButton(
              ...
              _row(Icons.code_rounded, 'Code', building.code),
              _row(Icons.business_rounded, 'Department', building.department.isNotEmpty ? building.department : 'None'),
              _row(Icons.layers_rounded, 'Floors', '${building.numberOfFloors}'),
              _row(Icons.meeting_room_rounded, 'Rooms', '$roomCount'),
              _row(Icons.calendar_today_outlined, 'Created', _fmt(building.createdAt)),
              _row(Icons.update_rounded, 'Last Updated', _fmt(building.updatedAt)),
              _row(Icons.fingerprint_rounded, 'ID', building.id),
